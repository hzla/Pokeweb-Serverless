"""Deterministically import the neutral-mood, location-independent US HGSS reactions."""
...
def build(rompath):
    ...
    if bytes(rom.idCode)!=b'IPKE':raise ValueError('Requires the inspected US HeartGold layout')
    ...
    for i,mid in enumerate(mids):
        words=message_words(messages,mid)
    ...
    manifest={'schemaVersion':1,'sourceGame':'HeartGold IPKE','mood':0,'rules':34,'messages':27,'motions':12,'emotes':7,'resourceMembers':14,'dataSha256':hashlib.sha256(data).hexdigest(),'dataBytes':len(data),'emotesSha256':hashlib.sha256(emotes).hexdigest(),'emotesBytes':len(emotes),'emotesCrc32':zlib.crc32(emotes),'sourceRuleIds':[r[0] for r in rules],'excludedOccurrenceRows':excluded,'texts':text_manifest}
    ASSETS.mkdir(exist_ok=True)
    (ASSETS/'interactions.bin').write_bytes(data);(ASSETS/'interaction-emotes.narc').write_bytes(emotes)
    (ASSETS/'interactions.json').write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+'\n')
    print(f'Imported {len(rules)} rules, {len(mids)} messages, {len(aids)} motions, {len(bids)} emotes; {len(data)} + {len(emotes)} bytes')
if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('rom',type=Path);build(p.parse_args().rom)
