...
from build import HERE, BUILD, TOOLS, SUFFIX, CONTRACT
...
subprocess.run([str(TOOLS/'arm-none-eabi-ld'),'-Ttext',hex(BASE),'-Tdata',hex(BASE+0x8000),'-e','PokewebFollowingObjectRow',str(BUILD/f'PokewebFollowingCore{SUFFIX}.elf'),'-o',str(linked)],check=True)
...
veneer=bytes(uc.mem_read(symbols[f'FULL_COPY_ARM9_0x{retail:08x}'],8))
...
actual=bytes(uc.mem_read(symbols[f'FULL_COPY_12_0x{addr:08x}'],size))
