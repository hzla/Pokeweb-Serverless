...
type FacilityMode = "sets" | "choices";
...
export function renderBattleFacilityEditor(project: ProjectState, root: HTMLElement, onDirty?: () => void): void {
  const setNarcs = getFacilitySetNarcOptions(project);
  const choiceNarcs = getFacilityChoiceNarcOptions(project);
  ...
  root.innerHTML = `
    <div class="pokemon-filter trainer-filter facility-filter">
      ...
      <div class="facility-tabs">
        ...
        <button class="btn -default facility-tab ${activeMode === "choices" ? "-active" : ""}" data-facility-mode="choices" type="button" ${choiceNarcs.length ? "" : "disabled"}>Choices</button>
      </div>
      ${renderArchiveSelect(setNarcs, choiceNarcs)}
  ...
  attachFacilityEvents(project, root, onDirty);
}

function renderArchiveSelect(setNarcs: FacilitySetNarcName[], choiceNarcs: FacilityChoiceNarcName[]): string {
  if (activeMode === "sets") {
    return `
...
function renderActivePanel(project: ProjectState): string {
  if (activeMode === "sets") {
  ...
  return renderChoicePanel(project, activeChoiceNarc);
}
...
function renderChoicePanel(project: ProjectState, narc: FacilityChoiceNarcName): string {
  const rows: string[] = [];
  ...
  return `
    <div class="pokemon-list spreadsheet facility-list" id="facility-choices">
      ...
      ${rows.join("") || `<div class="facility-empty">No matching choice records.</div>`}
    </div>
  `;
}
...
function editable(
  ...
  value: unknown,
  className: string,
  options: { autofill?: keyof ReturnType<typeof getFacilityAutofills>; type?: string } = {},
): string {
  ...
  return `<div autocorrect="off" data-facility-edit="${kind}" data-field-name="${field}" class="${className}" contenteditable="true"${autofill}${type}>${escapeHtml(String(value ?? ""))}</div>`;
}

function attachFacilityEvents(project: ProjectState, root: HTMLElement, onDirty?: () => void): void {
  root.querySelectorAll<HTMLButtonElement>("[data-facility-mode]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      renderBattleFacilityEditor(project, root, onDirty);
    });
  });
  ...
  root.querySelectorAll<HTMLSelectElement>("select[data-facility-edit]").forEach((field) => {
    field.addEventListener("change", () => {
      commitFacilityField(project, root, field, onDirty);
    });
  });
...
function commitFacilityField(project: ProjectState, root: HTMLElement, field: HTMLElement | HTMLSelectElement, onDirty?: () => void): void {
  ...
  try {
    ...
    else updateFacilityChoiceField(project, card.dataset.narc as FacilityChoiceNarcName, id, fieldName, value);
    onDirty?.();
    renderBattleFacilityEditor(project, root, onDirty);
  } catch (error) {
