...
|---|---|---|
| All four profiles | Exact binary contract, packaging/imports, module instruction/relocation checks (`verify_packaged.py`), hook anchors (`verify_anchor.py`), bundle size and manifest checks | `npm run following:test` for Pokeweb installer/model tests |
| Stock White 2 | `verify_continuity.py`, `verify_render.py`, `verify_surf.py`, `verify_land.py`, `verify_land_draw.py`, `verify_land_input.py`, `verify_cycle.py`, `verify_transition.py` | `verify_registry_stream.py ROM` for ROM-backed cache/FS cases; focused `verify_*.py` scripts as needed |
| Black 2 | `verify_profile_runtime.py black2 ROM` runs shared Surf, scene, land input/draw and transition harnesses with Black 2 bindings | `verify_black2_mount_binary.py` and targeted port diagnostics as needed |
...
| White2Upgrade | `verify_interactions.py`, `verify_render.py`, `verify_surf_upgrade.py`, `verify_scenes.py`, `verify_land_input.py` and packaged checks | `verify_registry_stream.py --profile white2upgrade ROM` for expanded species and stream validation; [Upgrade checklist](WHITE2UPGRADE-CHECKLIST.md) |
